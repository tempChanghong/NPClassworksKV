...
上述两个 main 都是对应功能分支的祖先。本报告若单独提交，会产生仅文档的后端新 HEAD；不要把它与受测代码 SHA 混淆。设备后续仅文档 HEAD `5771214` 的托管运行 `35563152422` 也已通过，运行代码仍对应下述 `0d9719c`。
...
2. **确认回退材料和现场配置。** 管理员核实现用 `DEPLOY_MODE` 是 shared 还是 standalone、对应 Compose 文件、项目名、工作目录、域名、反代及 `NPEP_ENABLED=false`；不能仅由前后端域名分离推断部署模式。确认当前健康，保留升级前数据库备份校验、前后端镜像 ID/标签和原部署 manifest。只回传核验结论及非秘密版本信息，不上传 env 文件。
...
- **确需还原数据库**：`rollback.sh:40-49` 先重 tag 旧镜像、调用当前工作区 restore，再检出旧代码。restore 必须通过仍在运行的可信 N1 容器完成换代际之后才能 dropdb。若后端已经停止或运行旧容器，当前流程会拒绝，而不是自动接管。即使单独运行可信 N1 helper 关闭了 epoch，现有 restore 仍会要求运行容器，因此不能把“已关 epoch”当成“恢复已就绪”。完整离线恢复步骤尚未实现和验收，是发布应急方案的待补项；不能改成忽略失败。
