...
组织预检还会读取当前数据库。相同学期、相同代码的教学空间不能通过导入更换类型或转移年级；已有作业目标引用或任课关系（包括停用的任课关系）的走班教学班不能更换科目。正式导入会在事务写入前重复检查，冲突通过现有的 422 校验结果返回，附带文档路径、教学空间 ID 和原因；整批导入不写入。

重命名仍然允许；没有作业目标引用和任课关系的空教学班仍可更换科目。确需改变教学空间用途时，请使用新代码创建并按既有流程处理旧教学空间。预检结果是当时的检查结果，不能代替正式导入时的核验。
