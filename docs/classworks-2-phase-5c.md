...
后端模板位于 `config/examples/newfires-high-school-organization.example.json`，管理页可以直接载入并编辑。模板包含高二 1 至 7 班：

- 1、4 班：语数外、物化生全部 `ADMIN_CLASS`，学生不会看到物化生走班选择。
- 2 班：政治随行政班，其余小科走班。
- 3 班：物理、化学随行政班，其余小科走班。
- 5、6、7 班：每班各固定一个小科，模板暂用化学、生物、地理，合计二理一文，其余小科走班。
- 每个走班科目提供两个教学班示例（A1/A2 或 B1/B2），并只关联授课模式为 `COURSE_GROUP` 的来源行政班。

五至七班的具体固定科目仍是根据当前沟通形成的可编辑示例；目前确定的是三个班每班各定一科、合计二理一文。不应在未核对最终对应科目和教学班名称前直接导入。管理页强制先 dry-run；后端仍会在正式导入时再次完整校验。
